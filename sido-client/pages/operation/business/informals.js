...
export async function getServerSideProps() {
  const res = await fetch(`http://localhost:5000/api/owners?formalized=false`);
  const data = await res.json();
  const title = "Informal businesses";
  let index = 1;
...
export default function InformalOwners({ list, title, description }) {

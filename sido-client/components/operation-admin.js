...
const actions = [
  { icon: <AttachMoneyIcon />, name: "Loan Manager" },
  { icon: <BusinessCenterIcon />, name: "BD Officer" },
];
...
export default function OperationAdmin({
  list,
  title,
  description,
  options,
  ...
  const handleAddOpen = (role) => {
    ...
    const role_name =
      role == 4 ? "Loan Manager" : "Business Development Officer";
        ...
        >
          {actions.map((action) => (
            <SpeedDialAction
              ...
              tooltipOpen
              onClick={() => {
                if (action.name === "BD Officer") {
                  handleAddOpen(5);
                } else if (action.name === "Loan Manager") {
                  handleAddOpen(4);
                }
              }}
            />
